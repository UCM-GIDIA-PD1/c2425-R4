...
def recordPeleas(peleadores,peleas):
    ...
    for idx, row in peleas.iterrows():
        ...
        rec1 = r[peleador1] if peleador1 in r else [0, 0, 0]
        rec2 = r[peleador2] if peleador2 in r else [0, 0, 0]
        ...
        peleas.loc[idx, 'Win_A'] = rec1[0] if rec1[0] >= 0 else 0
        peleas.loc[idx, 'Loss_A'] = rec1[1] if rec1[1] >= 0 else 0
        peleas.loc[idx, 'Draw_A'] = rec1[2] if rec1[2] >= 0 else 0
        
        peleas.loc[idx, 'Win_B'] = rec2[0] if rec2[0] >= 0 else 0
        peleas.loc[idx, 'Loss_B'] = rec2[1] if rec2[1] >= 0 else 0
